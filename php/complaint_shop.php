...
//mysqli_query($con, "LOCK TABLES login_l READ, user_credits_ucr READ, complaint_nearby_shops_cns WRITE");
...
if (is_null($json))
{
  ...
  if ($row = mysqli_fetch_array($result))
  {
    ...
    {
      $query_3 = "SELECT cns_id FROM complaint_nearby_shops_cns WHERE cns_ns_id=".sqlstrval($id)." AND cns_openid=".sqlstr($openid);
      ...
      {
        $query_4 = "SELECT COUNT(cns_id) AS total FROM complaint_nearby_shops_cns WHERE cns_ns_id=".sqlstrval($id);
        ...
        if ($row = mysqli_fetch_array($result))
        {
          ...
          {
            $query_5 = "INSERT INTO complaint_nearby_shops_cns (cns_ns_id,cns_openid,cns_time,cns_type,cns_content) VALUES (?,?,?,?,?)";
